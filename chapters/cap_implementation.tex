...
spectrum analyser a sinusoidal (Carrier) wave at 2.4 Ghz after all initialization
...
the FPGA design and is composed by two parts, a transmitting interface and a receiving
interface. The transmitting interface connects the DMA, which is reading data from the main
memory, with the DAC interface and the receiving interface connects the ADC output with the
DMA that writes on the main memory, in the subsections below the two interface will be more explored.
...
The DAC-DMA is implemented

Below it is possible to see a schematic of the interface:
