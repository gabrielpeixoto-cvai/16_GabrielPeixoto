...
%	\item[Microblaze] \hfill \\
  ...
  %interrupt input.
  %A processor was instantiated in the FPGA, the Microblaze. The Microblaze role
  %in the system developed in this work is basically to initialize all the
  %software drivers, configure such drivers and execute interrupt routines. %In
  %Figure \ref{fig:mbbd} it is possible to see a block schematic of microblaze
  %core.
...
%Note also the integrated circuit has a PLL, whose reference comes from the
%external differential port XTALp XTALn.
...
%There are two basic ways to input data into the AD9361, to do so it is necessary
