...
a more updated board and tool, thus the idea of porting the initial design from
...
\begin{enumerate}
    \item Analyse which ports to be connected
...
spectrum analyser a sinusoidal wave at 2.4 Ghz

%inserir figura do analisador de spectros

\section{Transmitting custom data}

After having the communication and initialization working properly, the next
step was to input custom data into the device and analyse its output.

There is two basic ways to input data into the AD9361, we need just to understand
...
is not so fast and reliable, but worked.
